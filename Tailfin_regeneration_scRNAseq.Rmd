---
title: "Tailfin_ regeneration_scRNAseq"
author: "Maya Emmons-Bell"
date: "3/31/2022"
output: html_document
---

QC, integration, regression, clustering, and data exploration of scRNAseq samples collected over a time course of zebrafish tailfin regeneration. Raw data from Hou et al., Science Advances, 2020. 
...
saveRDS(all.combined.integrated, file = "all.combined.integrated")
```
...
DimPlot(all.combined.integrated, group.by = "integrated_snn_res.0.3")
...
clusters <- DimPlot(all.combined.integrated, reduction = 'umap', group.by = 'integrated_snn_res.0.3', label = TRUE)
condition <- DimPlot(all.combined.integrated, reduction = 'umap', group.by = 'orig.ident')
condition|clusters
```
...
#dim plots
clusters <- DimPlot(all.combined.integrated, reduction = 'umap', group.by = 'integrated_snn_res.0.3', label = TRUE)
cell_cycle <- DimPlot(all.combined.integrated, reduction = 'umap', group.by = 'Phase')
cell_cycle|clusters
#or, ridge plots
...
DimPlot(all.combined.integrated, reduction = "pca")
...
gene <- FeaturePlot(all.combined.integrated, features = c("agr2"))
clusters <- DimPlot(all.combined.integrated, group.by = 'integrated_snn_res.0.3', label = TRUE)
gene|clusters


#assigning identities to clusters 
mesenchymal <- WhichCells(all.combined.integrated, idents = c(5)) %>% data.frame() %>% mutate(integrated_cell_type = "mesenchymal")
supep <- WhichCells(all.combined.integrated, idents = c(2, 6, 0, 7)) %>% data.frame() %>% mutate(integrated_cell_type = "superficial epithelium")
intep <- WhichCells(all.combined.integrated, idents = c(4, 3)) %>% data.frame() %>% mutate(integrated_cell_type = "intermediate epithelium")
basalep <- WhichCells(all.combined.integrated, idents = c(1, 8)) %>% data.frame() %>% mutate(integrated_cell_type = "basal epithelium")
hematopoietic <- WhichCells(all.combined.integrated, idents = c(9)) %>% data.frame() %>% mutate(integrated_cell_type = "hematopoietic")
...
all.combined.integrated <- RenameIdents(all.combined.integrated, `0` = "Superficial epithelium", `1` = "Basal epithelium", `2` = "Superficial epithelium",
    `3` = "Intermediate epithelium", `4` = "Intermediate epithelium", `5` = "Mesenchyme", `6` = "Superficial epithelium", `7` = "Superficial epithelium", `8` = "Basal epithelium", `9` = "Hematopoietic", `10` = "Mucosal-like", `11` = "Mucosal-like", `12` = "Hematopoietic")
#take a look 
DimPlot(all.combined.integrated, label=T)
...
#make cleaner DimPlots for figures 
DimPlot((all.combined.integrated), group.by = "orig.ident", pt.size = 1, label = F, label.size = 6) + 
...
#plot only one sample at a time
